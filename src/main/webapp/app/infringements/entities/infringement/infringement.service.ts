...
export class InfringementService {
  ...
  queryByDriverEmail(email: string, req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IInfringement[]>(this.resourceUrl + '/email?email=' + email, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
